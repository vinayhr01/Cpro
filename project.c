...
void bfast(char bfmenu[][100]) //Breakfast Menu Screen
{
    ...
    {
        printf("\nInvalid choice entered, Please enter the choice again!!!!\n\n");
        bfast(bfmenu);
    }
    repeatbf(bfmenu);
}
...
void display_bf_bill(char bfmenu[][100])
{
    ...
    printf("\t ITEMS \t\t Quantity \tPrice(in Rs./item)\t Total Price\n\n");
    for(i=0;i<7;i++)
    {
        if(purchased[i][1]!=0)
        {
            printf("\t %s    \t %d    \t\t",bfmenu[i],purchased[i][0]);
            printf("%d\t\t",bfrate[i][1]);
            printf("\t%d X Rs.%d (%s) = %d\n\n",purchased[i][0],bfrate[i][1],bfmenu[i],purchased[i][0]*bfrate[i][1]);
        }
    }
    printf("\t\t\t   GST=Rs.%0.2f\n",gst);
...
void display_lunch_bill(char lunchmenu[][100])
{
    ...
    for(i=0;i<7;i++)
    {
        if(purchased[i][1]!=0)
        {
            printf("\t %s    \t %d    \t\t",lunchmenu[i],purchased[i][0]);
            printf("%d\t\t",lunchrate[i][1]);
            printf("\t%d X Rs.%d (%s) = %d\n\n",purchased[i][0],lunchrate[i][1],lunchmenu[i],purchased[i][0]*lunchrate[i][1]);
        }
    }
    printf("\t\t\t   GST=Rs.%0.2f\n",gst);
    printf("\t\t\t   C-GST=Rs.%0.2f\n",cgst);
...
void display_dinner_bill(char dinnermenu[][100])
{
    ...
    for(i=0;i<7;i++)
    {
        if(purchased[i][1]!=0)
        {
            printf("\t %s    \t %d    \t\t",dinnermenu[i],purchased[i][0]);
            printf("%d\t\t",dinner_rate[i][1]);
            printf("\t%d X Rs. %d (%s) = %d\n\n",purchased[i][0],dinner_rate[i][1],dinnermenu[i],purchased[i][0]*dinner_rate[i][1]);
        }
